...
void setup() {
  ...
  //Sensor
  pinMode(A0, INPUT);
  pinMode(hardPress, INPUT_PULLUP);
  pinMode(lightPress, INPUT_PULLUP);
...
void loop() {
  if (startUpCounter == 0) {    //Only first time
    startUp();
  }

  //READ SENSORS
  
  int readLight = analogRead(lightSensor);
  Serial.println(readLight);
  int lightPressure = digitalRead(2);
  int hardPressure = digitalRead(3);
  
  lightPressure = map(lightPressure, 0, 1, 1, 0);
  hardPressure = map(hardPressure, 0, 1, 1, 0);
  ...
  if (alive == 0) {
    ...
    if (readLight < ambientLight + 10 && lifeCounter < lives) {
      ...
      Serial.println(readLight);
      delay(10); //instead of milis????
    }
       
  } else {
 
    //Map sensor value to conditions and state  
    if (hardPressure == 1 && lightPressure == 1) {                   // IF HARD?
      globalState = 1;

    } else if (lightPressure == 1 && hardPressure == 0) {                          //IF LIGHT?
      globalState = 2;
  
    } else if (readLight > ambientLight + lightTrim && lightPressure == 0) {            //0
      globalState = 3;

    } else {
      globalState = 4;          //IF nothing?
    }
